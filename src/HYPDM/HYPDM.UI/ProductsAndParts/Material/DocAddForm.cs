...
namespace HYPDM.WinUI.ProductsAndParts.Material
{
    ...
    {
        public DocAddForm(String p_MaterailId)
        {
            InitializeComponent();
        ...
        private void DocList_Init() {
            this.dgv_DocList.DataSource = m_MaterailService.GetDocList(m_Materail.MATERIALID,m_Materail.VERSION,0,null);
        ...
        private void tsb_query_Click(object sender, EventArgs e)
        {
            if (this.tsb_type.Text.Equals("编号")) {
                this.dgv_DocList.DataSource = m_MaterailService.GetDocList(m_Materail.MATERIALID, m_Materail.VERSION, 1, this.tsb_value.Text); 
            }
            if (this.tsb_type.Text.Equals("描述"))
            {
                this.dgv_DocList.DataSource = m_MaterailService.GetDocList(m_Materail.MATERIALID, m_Materail.VERSION, 2, this.tsb_value.Text); 
            }
            //DocList_Init();
        }
